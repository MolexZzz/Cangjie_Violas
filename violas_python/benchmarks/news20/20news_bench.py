"""
20 Newsgroups text retrieval benchmark

Refer to the process of caltech_bench.py and use text vectorization.
"""
import argparse
import os
import sys
...
from news20_feature_extract import extract_all_text_features_from_folders, KeyPredictor
...
def build_vector_database_for_test(root, test_size=0.1, random_state=42, build_milvus=False, use_milvus_lite=True, milvus_host="localhost", milvus_port="19530",build_qdrant=False, build_chroma=False,alpha=0.5, use_cache=True, cache_prefix=None, max_features=1024, n_components=256, vectorizer_method="tfidf"):
    """Build vector map table and display storage statistics
    Parameters:
        root: data set root directory
        ...
        max_features: TF-IDF maximum number of features (default 1024)
        n_components: Dimensionality reduction target dimension (default 256), None means no dimensionality reduction; it will participate in caching keys to avoid mixing with old caches
    ...
    # Cache file naming: 20news_<prefix>_ts0.1_rs42_a0.500.pkl
    ...
    cache_name = f"20news_{cache_prefix}_ts{test_size}_rs{random_state}_a{alpha:.3f}_vm{vectorizer_method}_dim{n_components}_folder.pkl"
    cache_file = os.path.join(cache_dir, cache_name)
    legacy_base = f"20news_{cache_prefix}_ts{test_size}_rs{random_state}_a{alpha:.3f}"
    cache_candidates = [
        cache_file,
        os.path.join(legacy_cache_dir, cache_name),
        os.path.join(legacy_cache_dir, f"{legacy_base}_dim{n_components}_folder.pkl"),
        os.path.join(legacy_cache_dir, f"{legacy_base}_folder.pkl"),
    ]
    ...
    if use_cache and os.path.exists(cache_file):
        ...
        # Step 1: Extract text features and complete train/test division (20news text interface, default TF-IDF + SVD dimensionality reduction to n_components)
        print(f"\nStep 1: train/test ({vectorizer_method})...")
        train_vectors_by_folder, train_descriptions_by_folder, test_data, _vectorizer = extract_all_text_features_from_folders(
            root, vectorizer_method=vectorizer_method, test_size=test_size, random_state=random_state, max_features=max_features, n_components=n_components
        )
        folders = sorted(train_vectors_by_folder.keys())
        print(f"Collected in total {len(folders)} folder data")

        # Step 2: Create KeyPredictor (share the vectorizer with the document to ensure that the key vectors are in the same space)
        ...
        # Step 3: The train/test division has been completed in extract_all_text_features_from_folders, no further splitting is needed here
        print(f"Train/Test The text feature extraction stage has been completed, and the test set has a total of {len(test_data)} folders")
...
def test_recall(vectormap: VectorMap, test_data, top_k=10, milvus_collection=None, milvus_id_map=None, qdrant_client=None, qdrant_id_map=None, chroma_collection=None, chroma_id_map=None,save_results=True, output_dir="outputs/news20", expansion_factor=10, beta=0.5, file_prefix=None, alpha=None, use_hdmg=False, hdmg_cluster_pool_size=64, hdmg_extra_hops=0, max_queries=None):
    ...
    print(f"total {len(all_test_samples)} test samples\n")
    ...
    for idx, sample in enumerate(tqdm(all_test_samples, desc="Under test"), 1):
        ...
        try:
            ...
            # 7. Chroma search logic
            chroma_results = None
            chroma_mixed_results = None
            if chroma_collection is not None and chroma_id_map is not None:
                # Pure vector search
                c_start = time.time()
                # Note: Chroma does not distinguish between mixed natively. We call the mixed search in database_build. Setting beta to 0 is a pure vector.
            ...
            # Calculate recall: beta<1, use doc_id/img_path to collect / |GT|; beta=1, use oracle GT key to collect, according to rank: hits@k / k
...
if __name__ == "__main__":
    _default_root = os.path.abspath(os.path.join(REPO_ROOT, "..", "dataset", "20_newsgroups"))

    parser = argparse.ArgumentParser(description="20 Newsgroups recall benchmark")
    parser.add_argument("--root", default=_default_root, help="Dataset root path")
    ...
    parser.add_argument("--vectorizer_method", type=str, default="tfidf", choices=["tfidf", "sentence_transformers"])
    ...
    # Recall test - multiple sets of parameters
    # Define the parameter combination to be tested [(expansion_factor, beta), ...]
    param_combinations = (
        [args.only_beta]
    ...
    for i, beta in enumerate(param_combinations):
        ...
        if not isinstance(stats, dict):
            stats = {
                'average_recall_mixed': 0.0, 'average_recall_representative': 0.0,
                'min_recall_mixed': 0.0, 'max_recall_mixed': 0.0,
                'min_recall_representative': 0.0, 'max_recall_representative': 0.0,
                'average_recall_python_flat': None, 'average_recall_milvus': None,
                'average_recall_milvus_mixed': None, 'average_recall_qdrant': None,
                'average_recall_qdrant_mixed': None, 'average_recall_chroma': None,
                'average_recall_chroma_mixed': None,
                'total_gt_time': 0.0, 'total_mixed_time': 0.0, 'total_representative_time': 0.0,
                'total_python_flat_time': None, 'total_milvus_time': None, 'total_milvus_mixed_time': None,
                'total_qdrant_time': None, 'total_qdrant_mixed_time': None, 'total_chroma_time': None, 'total_chroma_mixed_time': None,
                'avg_gt_time': 0.0, 'avg_mixed_time': 0.0, 'avg_representative_time': 0.0, 'avg_python_flat_time': None,
                'avg_milvus_time': None, 'avg_milvus_mixed_time': None, 'avg_qdrant_time': None,
                'avg_qdrant_mixed_time': None, 'avg_chroma_time': None, 'avg_chroma_mixed_time': None,
                'hdmg_avg_depth': None, 'hdmg_avg_hops': None, 'hdmg_avg_candidate_pool': None,
                'hdmg_avg_mixed_score_calls': None, 'hdmg_avg_t_entry_ms': None,
                'hdmg_avg_t_walk_ms': None, 'hdmg_avg_t_rerank_ms': None,
            }
