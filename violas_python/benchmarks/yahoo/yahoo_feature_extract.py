"""
Yahoo! Answers - extract_all_text_features_from_folders: train/test - KeyPredictor: sentence_transformers key
"""
...
def extract_all_text_features_from_folders(
    ...
    """
    Yahoo Answers train/test。 : train_vectors_by_folder, train_descriptions_by_folder, test_data, vectorizer
    """
    ...
    print(f"\nInitialize the feature extractor: {vectorizer_method} ({model_name}), max_features={max_features}...")
    vectorizer = Vectorizer(method=vectorizer_method, model_name=model_name, max_features=max_features)
    ...
    if vectorizer_method == 'tfidf':
        ...
        print("TF-IDF ， train/test...")
    else:
        vectorizer.fit_transform(all_texts)
        print(f"{vectorizer_method} ， train/test...")
...
class KeyPredictor:
    """KeyPredictor； vectorizer key （ tfidf）。"""

    def __init__(self, keys: List[str], prompt_template: str = "a question about {}", vectorizer=None):
        self.keys = keys
    ...
    @classmethod
    def from_key_lists(cls, keys: List[str], prompt_template: str = "a question about {}", vectorizer=None):
