"""
Ohsumed - beta mixed GT锛坰earch_python_flat_mixed锛?- 锛欻DMG銆丷epresentative銆丳ythonFlat銆丮ilvus銆丵drant銆丆hroma
"""
...
from ohsumed_feature_extract import extract_all_text_features_from_folders, KeyPredictor
...
def build_vector_database_for_test(
    ...
    cache_name = f"ohsumed_{cache_prefix}_ts{test_size}_rs{random_state}_a{alpha:.3f}_vm{vectorizer_method}_folder.pkl"
    cache_file = os.path.join(cache_dir, cache_name)
    cache_candidates = [
        cache_file,
        os.path.join(legacy_cache_dir, cache_name),
        os.path.join(legacy_cache_dir, f"ohsumed_{cache_prefix}_ts{test_size}_rs{random_state}_a{alpha:.3f}_{vectorizer_method}_folder.pkl"),
    ]
    ...
    if use_cache and os.path.exists(cache_file):
        ...
        if cache_obj.get("vectorizer") is not None:
            ...
            _predictor = KeyPredictor.from_key_lists(folders, vectorizer=cache_obj["vectorizer"])
            print(f"[Cache] VectorMap/TestData loaded, number of keys: {len(_predictor.get_all_keys())}")

    if vectormap is None:
        print(f"\nStep 1: ({vectorizer_method})...")
        ...
        print("\nStep 2: KeyPredictor ( TF-IDF )...")
        _predictor = KeyPredictor.from_key_lists(folders, vectorizer=_vectorizer)

        train_data_by_key = {}
        for folder in folders:
            train_data_by_key[folder] = {
                'vectors': train_vectors_by_folder[folder],
                'descriptions': train_descriptions_by_folder[folder]
            }
    ...
    for idx, sample in enumerate(tqdm(all_test_samples, desc="Under test"), 1):
        ...
        try:
            ...
            if qdrant_client is not None and qdrant_id_map is not None:
                q_start = time.time()
                query_vec = query_vector.tolist() if isinstance(query_vector, np.ndarray) else query_vector
                q_res = qdrant_client.query_points(collection_name="ohsumed", query=query_vec, limit=top_k)
                q_hits = q_res.points
                q_end = time.time()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Ohsumed recall benchmark")
    parser.add_argument("--sample_ratio", "-s", type=float, default=1.0, help="(0~1)")
    ...
    parser.add_argument("--root", default=None, help="锛?dataset/ohsumed-all-docs/ohsumed-all")
    ...
    root = args.root or os.path.abspath(os.path.join(REPO_ROOT, "..", "dataset", "ohsumed-all-docs", "ohsumed-all"))
    ...
    vectormap.build_hdmg(
        embedding_k=args.hdmg_embedding_k,
        semantic_intra_k=args.hdmg_semantic_intra_k,
    ...
    print(f": {len(test_data)} , {total_test} vectors")
    ...
    print(f"{'beta':<6} | {'mixed':<8} {'representative':<14} {'pyflat':<8} {'milvus':<8} {'milvus_mix':<10} {'qdrant':<8} {'chroma':<8}")
    print("-" * 100)
    for r in results_summary:
        ...
        print(f"{r['beta']:<6.2f} | {r['average_recall_mixed']:<8.4f} {r['average_recall_representative']:<14.4f} {pyflat:<8} {milvus:<8} {milvus_mix:<10} {qdrant:<8} {chroma:<8}")
    print(f"{'='*100}")
