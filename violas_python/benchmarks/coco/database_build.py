...
from coco_feature_extract import KeyPredictor
...
def build_milvus_collection_from_train_data(train_data_by_key, collection_name="flower_102", use_lite=True, host="localhost", port="19530", lite_uri=None):
    ...
    Parameters:
        ...
        port: Milvus server port (only if use_lite=False when used)
        lite_uri: Milvus Lite database path. Use ./milvus_lite_ when None{pid}.db锛屻€?: collection: Milvus Collection id_to_result: 锛?Milvus ID (key, group_name, vector_idx)
    """
    build_start_time = time.time()
    ...
    try:
        try:
            ...
            pass
        if use_lite:
            db_path = lite_uri or os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", f"milvus_lite_{os.getpid()}.db"))
    ...
    schema = CollectionSchema(fields, "Flower-102 vector collection")
...
def build_qdrant_collection_from_train_data(train_data_by_key, collection_name="flower-102", use_memory=True, host="localhost", port=6333):
...
def search_qdrant_mixed(client, collection_name, query_vector, query_key_vector, key_vectors, id_to_result, beta, top_k, candidate_factor=80):
    ...
    return out
def build_chroma_collection_from_train_data(train_data_by_key, collection_name="flower-102", use_persistent=False, chroma_path=None):
    """
    use_persistent: False When using memory mode, multi-process/multi-thread will not interfere with each other when running flower in parallel;
                    Use chroma_path persistence when True (multiple processes sharing the same path will overwrite each other's collections, causing Collection does not exist).
    chroma_path: use_persistent only=True Valid when ./chroma_db is used by default
    """
    if use_persistent:
        path = chroma_path or os.path.join(os.path.dirname(__file__), "..", "..", "chroma_db")
