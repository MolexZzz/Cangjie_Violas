"""
CUB-200-2011 Semantic recall benchmark of the dataset.

Consistent with caltech-bench: organize images by category folders, extract CLIP features, build VectorMap,
Use KeyPredictor to predict keys and evaluate Recall / Semantic Recall; support expansion_factor and beta parameter experiments.
"""
import os
import sys
import time
...
def build_vector_database_for_test(root, test_size=0.1, random_state=42, build_milvus=False, use_milvus_lite=True, milvus_host="localhost", milvus_port="19530",build_qdrant=False, build_chroma=False,alpha=0.5, use_cache=True, cache_prefix=None):
    ...
    # Cache file naming: CUB_<prefix>_ts0.1_rs42_a0.500.pkl
    ...
    cache_name = f"CUB_{cache_prefix}_ts{test_size}_rs{random_state}_a{alpha:.3f}_folder.pkl"
...
if __name__ == "__main__":
    import argparse
    _default_root = os.path.abspath(os.path.join(REPO_ROOT, "..", "dataset", "CUB_200_2011", "images"))
    parser = argparse.ArgumentParser(description="CUB-200-2011 recall benchmark")
    ...
    parser.add_argument("--hdmg_cluster_pool_size", type=int, default=9)
    ...
    if not os.path.isdir(root):
        ...
        print("Please download CUB-200-2011 and unzip it to dataset/CUB_200_2011/, ensuring that dataset/CUB_200_2011/images/ and its subfolders named according to categories exist.", file=sys.stderr)
        print(": http://www.vision.caltech.edu/datasets/cub_200_2011/ (CUB_200_2011.tgz)", file=sys.stderr)
        sys.exit(1)
    ...
    HDMG_SEMANTIC_BRIDGE_KEYS = 2
    HDMG_SEMANTIC_BRIDGE_PER_KEY = 1
    HDMG_CLUSTER_POOL_SIZE = args.hdmg_cluster_pool_size
    HDMG_EXTRA_HOPS = 0
    PYTHON_FLAT_MODE = "naive"
