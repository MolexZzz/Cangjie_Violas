...
from feature_extract import extract_all_features_from_folders, KeyPredictor
...
def build_vector_database_for_test(root, test_size=0.1, random_state=42, build_milvus=False, use_milvus_lite=True, milvus_host="localhost", milvus_port="19530",build_qdrant=False, build_chroma=False,alpha=0.5, use_cache=True, cache_prefix=None):
    ...
    # Cache file naming: caltech_<prefix>_ts0.1_rs42_a0.500.pkl
    ...
    cache_name = f"caltech_{cache_prefix}_ts{test_size}_rs{random_state}_a{alpha:.3f}_folder.pkl"
    ...
    if use_cache and os.path.exists(cache_file):
        ...
        # Step 1: Extract feature vectors of all images
        print("\nStep 1: Extract all image features...")
        all_vectors, all_descriptions = extract_all_features_from_folders(root)
...
if __name__ == "__main__":
    _default_root = os.path.abspath(os.path.join(REPO_ROOT, "..", "dataset", "caltech-101", "101_ObjectCategories"))
    parser = argparse.ArgumentParser(description="Caltech-101 recall benchmark")
    ...
    # Define the parameter combinations to be tested
    param_combinations = (
        [args.only_beta]
    ...
    for i, beta in enumerate(param_combinations):
        ...
        if not isinstance(stats, dict):
            stats = {
                ...
                'average_recall_chroma_mixed': None,
                'avg_mixed_time': None, 'avg_representative_time': None, 'avg_python_flat_time': None,
