...
def build_milvus_collection_from_train_data(train_data_by_key, collection_name="caltech_101", use_lite=True, host="localhost", port="19530"):
    ...
    schema = CollectionSchema(fields, "Caltech-101 vector collection")
...
def build_qdrant_collection_from_train_data(train_data_by_key, collection_name="caltech_101", use_memory=True, host="localhost", port=6333):
...
def search_qdrant_mixed(client, collection_name, query_vector, query_key_vector, key_vectors, id_to_result, beta, top_k, candidate_factor=80):
    ...
    return out
def build_chroma_collection_from_train_data(train_data_by_key, collection_name="caltech_101"):
    print("\n=== Chroma Collection () ===")
...
def search_python_flat_index(index_data, query_vector, top_k=10, mode="naive", return_stats=False):
    """
    Python IVF-FLAT 锛?nprobe 锛夛紝 [(img_path, key, distance), ...]
    """
    timings_ms = {}
    ...
    # Fallback: full table scan without IVF
